...
public class BezierQuadratic extends ArcCourbe 
{
	...
	public double projeteOrthogonal(Vecteur p) throws ExceptionGeometrique
	{
		try
		{
			...
			double x[] = MesMaths.resoutEquationDegre3(a0, a1, a2, a3);

			x = filtre(x, 0, 1);

			double distancesCarrees[] = new double[x.length];

			int i;
			for( i = 0; i <x.length; ++i) distancesCarrees[i] = Vecteur.difference(this.evalue(x[i]), p).normeCarree();

			int iMin = indiceDuMin(distancesCarrees);
	...
	public double [] intersectionDroite(final Vecteur A, final Vecteur n)
	{
		...
		double t[] = MesMaths.resoudre(a2, a1, a0);
