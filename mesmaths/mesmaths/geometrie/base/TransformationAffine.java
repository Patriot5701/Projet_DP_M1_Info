...
public class TransformationAffine
{
	...
	{
		/*Matrice R = Matrice.cr�eRotation(angle);
Vecteur u = centre.difference(R.produit(centre));
return new TransformationAffine(R, u);
		 */
		return TransformationAffine.creeTransformationAffineAvecPointInvariant(Matrice.creeRotation(angle), centre);
	}


	/**
	 * cr�e l'homoth�tie affine de coefficient "k" et de centre "centre" 
	 * 
	 * Soit P un point quelconque et P' son image par la transfo T cr��e. Soient k le coeff et C le centre, alors T respecte la condition suivante :
	 *  vecteur(CP') = k*vecteur(CP')
	 * 
	 * @param k : coefficient de l'homoth�tie
	 * @param centre : centre de l'homoth�tie
	 * 
	 * */
	public static TransformationAffine creeHomothetie(double k, Vecteur centre)
	{
		/*Matrice R = Matrice.cr�eMatriceDiagonale(k, k);
Vecteur u = centre.difference(R.produit(centre));
return new TransformationAffine(R, u);*/

		return TransformationAffine.creeTransformationAffineAvecPointInvariant(Matrice.creeMatriceDiagonale(k, k), centre);
	}

	/**
	 * cr�e une transformation affine avec point invariant
	 * 
	 * @param M : matrice quelconque
	 * @param pointInvairant : le point invariant
	 * */
	public static TransformationAffine creeTransformationAffineAvecPointInvariant(Matrice M, Vecteur pointInvariant)
	{
